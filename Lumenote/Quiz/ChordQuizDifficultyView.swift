...
/// Chooses difficulty and length, then runs the chord quiz in place.
/// The quiz is not pushed, so its back button returns to the feature menu.
struct ChordQuizDifficultyView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: LumenoteSpacing.section) {
                ...
                VStack(spacing: LumenoteSpacing.md) {
                    ForEach(ChordQuizDifficulty.allCases) { difficulty in
                        Button {
    ...
    private func difficultyRow(_ difficulty: ChordQuizDifficulty, isSelected: Bool) -> some View {
    ...
    private var startButton: some View {
        Button {
            model = ChordQuizModel(difficulty: selectedDifficulty, questionLimit: questionCount)
        } label: {
            Text("시작")
                .font(LumenoteFont.body(.bold))
    ...
    private func result(_ model: ChordQuizModel) -> some View {
        ScrollView {
            VStack(alignment: .leading, spacing: LumenoteSpacing.section) {
                Text("퀴즈 풀이 결과")
