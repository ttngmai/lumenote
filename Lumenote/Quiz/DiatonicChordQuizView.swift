...
struct DiatonicChordQuizView: View {
    ...
    private var promptCard: some View {
        VStack(spacing: LumenoteSpacing.xl) {
            ...
            case .completeRomanPattern:
                romanPatternPrompt
            case .identifyQuality, .identifyNonDiatonic:
                EmptyView()
            }
        }
    ...
    private var promptTitleView: some View {
        highlightedPrompt(
            ...
            highlights: model.question.promptHighlights
        )
    ...
    private var romanPatternPrompt: some View {
        Text(romanPatternAttributed)
            .frame(maxWidth: .infinity)
            .multilineTextAlignment(.center)
            .fixedSize(horizontal: false, vertical: true)
            .accessibilityLabel(romanPatternAccessibility)
    }

    private var romanPatternAttributed: AttributedString {
        var result = AttributedString()
        let tokens = model.question.promptTokens
        for (index, token) in tokens.enumerated() {
            var piece: AttributedString
            switch token {
            case .roman(let name):
                piece = AttributedString(name)
                piece.font = .system(size: 20, weight: .bold)
                piece.foregroundColor = .primary
            case .blank:
                piece = AttributedString("?")
                piece.font = .system(size: 20, weight: .bold)
                piece.foregroundColor = palette.minor
            }
            result.append(piece)

            if index < tokens.count - 1 {
                var separator = AttributedString(" · ")
                separator.font = LumenoteFont.callout(.semibold)
    ...
    private var romanPatternAccessibility: String {
        model.question.promptTokens.map { token in
            switch token {
            case .roman(let name): return name
