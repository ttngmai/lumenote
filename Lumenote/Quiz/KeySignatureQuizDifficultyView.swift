...
/// Chooses difficulty and length, then runs the key-signature quiz in place.
/// The quiz is not pushed, so its back button returns to the feature menu.
struct KeySignatureQuizDifficultyView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: LumenoteSpacing.section) {
                ...
                VStack(spacing: LumenoteSpacing.md) {
                    ForEach(KeySignatureQuizDifficulty.allCases) { difficulty in
                        Button {
    ...
    private func difficultyRow(_ difficulty: KeySignatureQuizDifficulty, isSelected: Bool) -> some View {
    ...
    private var startButton: some View {
        Button {
            model = KeySignatureQuizModel(difficulty: selectedDifficulty, questionLimit: questionCount)
        } label: {
            Text("시작")
                .font(LumenoteFont.body(.bold))
    ...
    private func result(_ model: KeySignatureQuizModel) -> some View {
        ScrollView {
            VStack(alignment: .leading, spacing: LumenoteSpacing.section) {
                Text("퀴즈 풀이 결과")
                ...
                Button {
                    self.model = KeySignatureQuizModel(
